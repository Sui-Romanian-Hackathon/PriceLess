...
/**
 * Extracts the Object ID of the newly created User from the transaction response.
 * @param result The full response object after executing the transaction block.
 * @returns The Object ID (string) of the created User, or null if not found.
 */
export function extractUserObjectId(result: SuiTransactionBlockResponse): string | null {
    if (!result.objectChanges) {
        return null;
    }

    // Look for a 'created' object change that matches the User type.
    const createdUserChange = result.objectChanges.find(change => {
        return (
            change.type === 'created' &&
    ...
    if (createdUserChange && 'objectId' in createdUserChange) {
        return createdUserChange.objectId;
    }
