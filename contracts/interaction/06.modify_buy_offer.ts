...
const NEW_PRICE = 40; // New price in RON tokens

async function modifyBuyOffer() {
    console.log(`Modifying Buy Offer`);

    try {
        const { client, keypair, address } = await initializeSui(NETWORK);

        console.log(`📝 Modifying buy offer from address: ${address}`);
        ...
        console.log(`  New Price: ${NEW_PRICE} RON`);
        ...
        console.log(`📞 Calling modify_buy_offer...`);
        ...
        // Execute transaction
        console.log('📤 Submitting modify_buy_offer transaction...');
        ...
        const success = displayTransactionResults(result, 'Modify Buy Offer transaction');

        if (success) {
            console.log(`🎉 Successfully modified buy offer!`);
        ...
        console.error('Error modifying buy offer:', error);
        process.exit(1);
    }
}
